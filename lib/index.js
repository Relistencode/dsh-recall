...
export function apply(ctx, config = {}) {
  ...
  // Background warm-up timer (optional timer service; when absent, semantic
  // vectors still accumulate through the per-call budget in execute()).
  const timerService = typeof ctx.get === 'function' ? ctx.get('timer') : null
  if (timerService && typeof timerService.setInterval === 'function') {
    const timer = timerService.setInterval(() => {
      warmupTick(getEmbedder(), getFuzzy(), warmup).catch(() => {})
    }, 30000)
    if (typeof ctx.effect === 'function') {
      ctx.effect(() => () => {
        try {
