...
**v1 · 现在** —— 装好即用:字面、模糊、语义三层检索,完全本地离线,不打扰你。

**v2 · 更懂"那件事"**
- **顺着聊**:从一条回忆出发,沿着当时的对话往下翻——像翻聊天记录一样看完整段
- **时间过滤**:只想回忆"上个月""那几天"发生的事
- **一次想起整件事**:同一件事散在多处时,合并成完整来龙去脉,而不是零散碎片
- **记忆锚点**:自动压缩发生时,把摘要登记为记忆锚点——回忆时顺着锚点,原文依然原样可取

**v3 · 更懂"你"**
- **主题化回忆**:结果按话题归拢,一段时期聊过什么一目了然
- **记忆沉淀**:从反复讨论中提炼你的设定与决策,沉淀为长期记忆的一部分

(远期:评估更好的压缩机制——只评估,不动 DSH 核心。)
